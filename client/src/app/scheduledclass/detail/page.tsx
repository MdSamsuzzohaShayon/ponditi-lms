  ...
  const acceptRequestHandler = async (are: React.SyntheticEvent) => {
    ...
    try {
      ...
      if (response.status === 200 || response.status === 202) {
        ...
        if(socket)await socket?.emit('update-notification-from-client', newDataObj);
          ...
          {isLoading ? (
            ...
            <>
              ...
              <button type="button" className="btn btn-primary" onClick={startClassHandler}>
                ...
                {singleScheduledClass.status === StatusEnum.FINISH_CLASS && <Review singleScheduledClass={singleScheduledClass as CreatedScheduledClassIn} />}
              </div>
            </>
          )}
        </div>

...
function ExamDetailForm() {
  ...
  const educationSingleExam = useAppSelector((state) => state.education.educationSingleExam);
  ...
  const addNewExamHandler = (e: React.SyntheticEvent) => {
    e.preventDefault();
  ...
  const euInputChangeHandler = (e: React.ChangeEvent<HTMLInputElement>, examId: number) => {
    // @ts-ignore
  ...
  const euInputRunningStudyHandler = (e: React.ChangeEvent<HTMLInputElement>, examId: number) => {
    // @ts-ignore
    const educationItem = educationUpdateList.find((eui) => eui.id === examId);
    // @ts-ignore
    const educationObj = { ...educationItem };
    educationObj[e.target.name] = e.target.checked;
  ...
  const euItemCloseHandler = (e: React.SyntheticEvent, examId: number) => {
    e.preventDefault();

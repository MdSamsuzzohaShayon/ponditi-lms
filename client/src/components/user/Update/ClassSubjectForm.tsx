...
function ClassSubjectForm() {
  const [userTuitionmIdList, setUserTuitionmIdList] = useState<number[]>([]);
  const [userClassTypeIdList, setUserClassTypeIdList] = useState<number[]>([]);
  const [userSubjectIdList, setUserSubjectIdList] = useState<number[]>([]);
  ...
  const displayClassType = useAppSelector((state) => state.classtype.displayClassType);
  const subjectList = useAppSelector((state) => state.subject.subjectList);
  const displaySubject = useAppSelector((state) => state.subject.displaySubject);
  ...
  const selectDefaultCheckbox = (list: UserSelectable[], itemId: number): boolean => {
    try {
  ...
  const rearrangeSubjectList = (selectedClassTypeList: ClassType[]): number[] => {
    const newSubjectIds: number[] = [];

    selectedClassTypeList.forEach((ct) => {
      ct.Subjects.forEach((sub) => {
  ...
  const removeHiddenCTFromSelection = (uniqueClassTypeIds: number[]) => {
    if (selectedClasstypeList.length > 0) {
      const filtered = selectedClasstypeList.filter((id) =>
        // @ts-ignore
        uniqueClassTypeIds.includes(id)
      );

      dispatch(setUpdateUser({ ClassTypeId: filtered }));
      dispatch(setSelectedClasstype(filtered));
    }
  };

  // Not working properly
  const removeHiddenSubFromSelection = (uniqueSubjectIds: number[]) => {
    if (selectedSubjectList.length > 0) {
      const filtered = selectedSubjectList.filter((id) =>
        // @ts-ignore
        uniqueSubjectIds.includes(id)
      );

      dispatch(setUpdateUser({ SubjectId: filtered }));
      dispatch(setSelectedSubject(filtered));
    }
  };
            ...
            <div className="row py-3">
              {tuitionmList.map((tm, idx) => (
                <div className="col-md-3 d-flex justify-content-end align-items-center flex-row-reverse" key={idx}>
                  {/* // @ts-ignore */}
                  <label className="fs-6 fw-light" htmlFor={String(tm.id)}>
            ...
            <div className="card-body">
              ...
              <div className="row my-3">
                {vlsiableClassTypeList().map((ctl, idx) => (
                  <div className="col-md-3 d-flex justify-content-end align-items-center flex-row-reverse" key={idx}>
                    <label className="fs-6 fw-light" htmlFor={String(ctl.id)}>
            ...
            <div className="card-body">
              ...
              <div className="row py-3">
                {vlsiableSubjectList().map((sub, idx) => (
                  <div className="col-md-3 d-flex justify-content-end align-items-center flex-row-reverse" key={idx}>
                    <label className="fs-6 fw-light" htmlFor={String(sub.id)}>

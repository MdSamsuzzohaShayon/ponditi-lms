...
// config and utils
import { scheduledclassStatus, AWS_S3_URL } from '@/config/keys';
...
function Detail({ userDetail, update, search, userId }: DetailPropsInterface) {
  ...
  const editPartToUpdateHandler = (epse: React.SyntheticEvent, partNum: number) => {
    ...
    // if(userId)router.push({ pathname: '/user/update', query: { userId } });
    router.push(`/user/update/?userId=${userId}`);
  };
  ...
  const headToSendRequestHandler = (htsre: React.SyntheticEvent) => {
    ...
    const classAndSubject: ClassAndSubjectInterface = { receiverId: parseInt(String(receiverId), 10) };
      ...
      {userDetail && (
        <>
          <div className="row">
            <div className="col-12 d-flex justify-content-center align-items-center flex-column mt-4">
              {userDetail.name && (
                <div className="text-center">
                  ...
                  {userDetail?.role === UserRoleEnum.TEACHER && userDetail.experience && <p className="p-0 mb-3">{userDetail.experience} years Experience</p>}
                </div>
              )}
            ...
            {update && userDetail?.role === UserRoleEnum.TEACHER && (
          ...
          {/* tution detail start  */}
          {userDetail?.role === UserRoleEnum.TEACHER && (
            <div className="row  mb-3 pb-3">
              <div className="heading d-flex justify-content-between align-items-center row pb-3">
          ...
          {(userDetail?.role ===  UserRoleEnum.TEACHER || userDetail?.role ===  UserRoleEnum.STUDENT) && (
            <div className="row  mb-3 pb-3">
              <div className="col-12 d-flex justify-content-between row align-items-center my-3">
                <h3 className="h5 w-fit">{userDetail.role ===  UserRoleEnum.TEACHER ? 'Preffered Subjects' : 'Mediums & Class'}</h3>
                {update && (
              ...
              <div className="body-content">
                {userDetail.role ===  UserRoleEnum.TEACHER ? (
                        ...
                        <div className="d-flex flex-wrap pe-3">
                          {userTuitionmList.map((us, usi) => (
                            ...
                            <p className="fw-semibold" key={us?.id}>
                              {/* //@ts-ignore */}
                              {/* {usi === userTuitionmList.length - 1 ? us.name : `${us.name}, `} &nbsp; // temp */}
                            </p>
                          ))}
                        </div>
                      </div>
                        ...
                        <div className="d-flex flex-wrap pe-3">
                          {userClassTypes.map((uct, ucti) => (
                            ...
                            <p className="fw-semibold" key={uct?.id}>
                              {/* //@ts-ignore */}
                              {/* {ucti === userClassTypes.length - 1 ? uct.name : `${uct.name}, `} &nbsp; // temp */}
                            </p>
                          ))}
                        </div>
                      </div>
                      ...
                      <div className="col-md-4">
                        ...
                        <div className="d-flex flex-wrap pe-3">
                          {userSubjects.map((us, usi) => (
                            ...
                            <p className="fw-semibold" key={us?.id}>
                              {/* //@ts-ignore */}
                              {/* {usi === userSubjects.length - 1 ? us.name : `${us.name}, `} &nbsp; // temp */}
                            </p>
                          ))}
                        </div>
                      </div>
                  ...
                  <div className="student-med-cls row">
                    <div className="col-md-4">
                      {userTuitionmList[0] && <p className="text-capitalize">Medium: 
                        {/* {userTuitionmList[0].name} // temp */} 
                        </p>}
                      {userClassTypes[0] && <p className="text-capitalize">Class:
                         {/* {userClassTypes[0].name} // temp */} 
                         </p>}
                    </div>
                  </div>
                )}
          ...
          {/* Exam detail start  */}
          {authUserInfo.id !== null && userDetail.role ===  UserRoleEnum.TEACHER && (
            <div className="row  mb-3 pb-3">
              <div className="heading d-flex justify-content-between align-items-center row pb-3">
                    ...
                    <div className="card rounded-1">
                      ...
                      <div className="card-body">
                        <p className="card-text">
                          Major : <span className="fw-semibold">{
                          // uel?.major // temp
                          }</span>{' '}
                        </p>
                        <p className="card-text">
                          Institution: <span className="fw-semibold">{
                          // uel?.institution // temp
                          }</span>
                        </p>
                        <p className="card-text">
                          {/* Passing Year: <span className="fw-semibold">{uel.running_study ? 'Running' : uel?.passing_year}</span>{' '}  // temp*/}
                        </p>
                      </div>
                    </div>
                  </div>
          ...
          {/* Feedback start  */}
          {userDetail.role ===  UserRoleEnum.TEACHER && userReviews.length > 0 && (
            <div className="row  mb-3 pb-3">
              <div className="heading d-flex justify-content-between align-items-center row pb-3">

...
// Socket
import { useSocket } from '@/lib/ThemeProvider';
...
function SendRequest() {
  ...
  const inputNumChangeHandler = (e: React.ChangeEvent<HTMLSelectElement | HTMLInputElement>) => {
    e.preventDefault();
    dispatch(setInitializeSchedule({ [e.target.name]: parseInt(e.target.value, 10) }));
  };
  ...
  const dateChangeHandler = (e: React.ChangeEvent<HTMLSelectElement | HTMLInputElement>, detail: any) => {
    e.preventDefault();
    // console.log(detail);

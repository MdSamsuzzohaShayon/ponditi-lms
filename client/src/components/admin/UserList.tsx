...
function UserList(props) {
  ...
  const rejectedUserHandler = async (e: React.SyntheticEvent, userId: number) => {
    e.preventDefault();
    try {
      ...
      if (response.status === 202) {
        ...
        // dispatch(fetchAllUsersByAdmin(null)); // temp
      }
    } catch (error) {
      console.log(error);
      // @ts-ignore
  ...
  const acceptUserHandler = async (e: React.SyntheticEvent, userId: number) => {
    e.preventDefault();
    try {
      dispatch(toggleLoading(true));
  ...
  const changeTabItemHandler = (e: React.SyntheticEvent, itemName: string) => {
    e.preventDefault();
        ...
        {adminUserTabElement.map((aut) => (
          <li key={aut.id} className="nav-item">
            <button
              ...
              onClick={(e) => changeTabItemHandler(e, aut.name)}
            >
              {aut.text}
            </button>
          </li>

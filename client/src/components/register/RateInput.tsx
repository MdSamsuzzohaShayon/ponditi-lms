...
function RateInput(props) {
  ...
  const inputPriceChangeHandler = (ipce: React.ChangeEvent<HTMLInputElement>) => {
    ...
    switch (selectedStyleItem) {
      ...
      case sl: {
        dispatch(setRegisterableUser({ sl_rate: parseInt(String(rate), 10) }));
        // @ts-ignore
      ...
      case tl: {
        dispatch(setRegisterableUser({ tl_rate: parseInt(String(rate), 10) }));
        // @ts-ignore
  ...
  const inputTuitionStyleErr = useMemo(() => {
